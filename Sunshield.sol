                      ...
                      function swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        ...
                        function swapExactETHForTokensSupportingFeeOnTransferTokens(
                          ...
                          */
                          ...
                          // File: contracts\SunShieldBase.sol


                          // Base class that implements: BEP20 interface, fees & swaps
                          abstract contract SunShieldBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
                            // MAIN TOKEN PROPERTIES
                            string private constant NAME = "Sunshield ";
                            string private constant SYMBOL = "SNSD";
                            ...
                            uint8 private _sunShieldBNBFee;
                            uint8 private  _devWalletFee;
                            uint8 private _moonShieldRewardFee;




                            uint256 private constant _totalTokens = 1000000000000000 * 10**DECIMALS;	//1 quadrillion total supply
                            ...
                            address public constant BURN_WALLET = 0x75Cb7cc29Cc9489A85E14744391Df17Dc8cA3746;//0x000000000000000000000000000000000000dEaD; //The address that keeps track of all tokens burned
                            uint256 private _tokenSwapThreshold = _totalTokens / 1000; //There should be at least 0.0001% of the total supply in the contract before triggering a swap
                            ...
                            // addresses where tax deducted is forwarded to
                            address private _sunShieldPoolAddress;
                            address private  _devWalletAddress;
                            address private _moonShieldPoolAddress;
                            ...
                            uint256 public _sunShieldBNBTax;
                            uint256 public  _devWalletFeeTax;
                            uint256 public _moonShieldRewardTax;

                            uint8 public _moonShieldPoolExtra;
                            uint8 public _buyBackFee;
                            uint256 public minClaimBNBFloor = 0.2 ether;
                            uint256 public minClaimBNBCeil = 0.34 ether;
                            uint256 public midClaimBNBFloor = 0.35 ether;
                            uint256 public midClaimBNBCeil = 0.49 ether;
                            uint256 public maxClaimBNBFloor = 0.5 ether;
                            ...
                            constructor (address routerAddress) {
                              ...
                              // Initialize PancakeSwap V2 router and SNSD <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
                              setPancakeSwapRouter(routerAddress);

                              // 4% liquidity fee, 12% reward fee, 4% additional sell fee
                              setFees(12, 4, 2, 1, 4);
                            ...
                            function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
                              ...
                              if (isTransferLimited(sender, recipient)) {
                                require(amount <= _tokenSwapThreshold, "Transfer amount exceeds the maximum allowed");
                              }
                            ...
                            function executeSwap(uint256 amount) private {
                              ...
                              uint256 tokensForSunShieldPool = amount * _sunShieldBNBFee/ _poolFee;
                              uint256 tokensForDevWallet = amount * _devWalletFee/_poolFee;
                              uint256 tokensForMoonShieldPool = amount * _moonShieldRewardFee/_poolFee;
                              ...
                              uint256 tokensToSwap = tokensReservedForReward + tokensReservedForReward;
                              ...
                              uint256 bnbToBeAddedToLiquidity = bnbSwapped * tokensReservedForLiquidity / tokensToSwap;
                              (,uint bnbAddedToLiquidity,) = _pancakeswapV2Router.addLiquidityETH{value: bnbToBeAddedToLiquidity}(address(this), tokensReservedForLiquidity, 0, 0, _autoLiquidityWallet, block.timestamp + 360);
                              uint256 bnbForSunShieldPool = bnbSwapped * tokensForSunShieldPool / tokensToSwap;
                              uint256 bnbforDevWallet = bnbSwapped * tokensForDevWallet / tokensToSwap;
                              uint256 bnbForMoonShield = bnbSwapped * tokensForMoonShieldPool / tokensToSwap;
                              _sunShieldBNBTax += bnbForSunShieldPool;
                              _devWalletFeeTax += bnbforDevWallet;
                              _moonShieldRewardTax += bnbForMoonShield;
                              // Keep track of how many BNB were added to liquidity this way
                              _totalBNBLiquidityAddedFromFees += bnbAddedToLiquidity;

                              emit Swapped(tokensToSwap, bnbSwapped, tokensReservedForLiquidity, bnbToBeAddedToLiquidity);
                            }


                            // This function swaps a {tokenAmount} of SNSD tokens for BNB and returns the total amount of BNB received
                            function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
                              uint256 initialBalance = address(this).balance;

                              // Generate pair for SNSD -> WBNB
                            ...
                            // Returns how many more $SNSD tokens are needed in the contract before triggering a swap
                            ...
                            function setFees(uint8 sunShieldBNBFee, uint8 liquidityFee, uint8 devWalletFee, uint8 moonShieldRewardFee,  uint8 additionalSellFee) public onlyOwner {
                              require(liquidityFee >= 1 && liquidityFee <= 4, "Liquidity fee must be between 1% and 4%");
                              uint8 rewardFee = sunShieldBNBFee + moonShieldRewardFee + devWalletFee;
                              require(rewardFee >= 1 && rewardFee <= 15, "Reward fee must be between 1% and 15%");
                              require(additionalSellFee <= 4, "Additional sell fee cannot exceed 4%");
                              require(sunShieldBNBFee + liquidityFee + devWalletFee + moonShieldRewardFee + additionalSellFee <= 23, "Total fees cannot exceed 23%");

                              _liquidityFee = liquidityFee;
                              _sunShieldBNBFee = sunShieldBNBFee;
                              _moonShieldRewardFee = moonShieldRewardFee;
                              _rewardFee = rewardFee;
                              _additionalSellFee = additionalSellFee;
                              _moonShieldPoolExtra = additionalSellFee/2;
                              _buyBackFee  = additionalSellFee/2;
                        ...
                        function SunShieldPoolAddress() public view returns (address) {
                          return _sunShieldPoolAddress;
                        }


                        function setSunShieldPoolAddress(address sunShieldPoolAddress) public onlyOwner {
                          _sunShieldPoolAddress = sunShieldPoolAddress;
                        }
                        function DevWalletAddress() public view returns (address) {
                          return _devWalletAddress;
                        }


                        function setDevWalletAddress(address devWalletAddress) public onlyOwner {
                          _devWalletAddress = devWalletAddress;
                        }

                        function MoonShieldPoolAddress() public view returns (address) {
                          return _moonShieldPoolAddress;
                        }


                        function setMoonShieldPoolAddress(address moonShieldPoolAddress) public onlyOwner {
                          _moonShieldPoolAddress = moonShieldPoolAddress;
                        }
                        ...
                        function sunShieldBNBTax() public view returns (uint256) {
                          return _sunShieldBNBTax;
                        }

                        function devWalletFeeTax() public view returns (uint256) {
                          return _devWalletFeeTax;
                        }

                        function moonShieldRewardTax() public view returns (uint256) {
                          return _moonShieldRewardTax;
                        }
                        ...
                        // File: contracts\SunShield.sol

                        // Implements rewards & burns
                        contract SunShield is SunShieldBase {

                          // REWARD CYCLE
                          uint256 private _rewardCyclePeriod = 4 hours; // The duration of the reward cycle (e.g. can claim rewards once every four hrs)
                          ...
                          uint256 private _totalBNBAsSNSDClaimed; // The total number of BNB that was converted to SNSD and claimed by all addresses
                          mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
                          mapping(address => uint256) private _bnbAsSNSDClaimed; // The amount of BNB converted to SNSD and claimed by each address
                          ...
                          mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SNSD automatically
                          ...
                          uint256 private _mainBnbPoolSize = 2000 ether; // Any excess BNB after the main pool will be used as reserves to ensure consistency in rewards
                          ...
                          uint256 private _gradualBurnTimespan = 4 hours; //Burn every 1 day by default
                          ...
                          //uint256 private _sunShieldPoolCharge =
                          ...
                          bool private _reimburseAfterSNSDClaimFailure; // If true, and SNSD reward claim portion fails, the portion will be given as BNB instead
                          ...
                          constructor (address routerAddress) SunShieldBase(routerAddress) {
                          ...
                          function onActivated() internal override {
                            ...
                            setReimburseAfterSNSDClaimFailure(true);
                          ...
                          function updateAutoClaimQueue(address user) private {
                            ...
                            function claimReward(address user) public {
                              ...
                              require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough SNSD balance");
                            ...
                            function doClaimReward(address user) private returns (bool) {
                              ...
                              (uint256 claimBnb, uint256 claimBnbAsTokens, uint poolCharge) = calculateClaimRewards(user);

                              bool tokenClaimSuccess = true;
                              // Claim SNSD tokens
                              if (!claimSNSD(user, claimBnbAsTokens)) {
                                // If token claim fails for any reason, award whole portion as BNB
                                if (_reimburseAfterSNSDClaimFailure) {
                                ...
                                //sendToSunShieldPool(poolCharge);
                                _sunShieldBNBTax += poolCharge;
                              ...
                              function claimBNB(address user, uint256 bnbAmount) private returns (bool) {
                                ...
                                function claimSNSD(address user, uint256 bnbAmount) private returns (bool) {
                                  if (bnbAmount == 0) {
                                    return true;
                                  }
                                  ...
                                  _bnbAsSNSDClaimed[user] += bnbAmount;
                                  _totalBNBAsSNSDClaimed += bnbAmount;
                                  return true;
                                }
                                ...
                                function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
                                  ...
                                  if (balance == 0) {
                                    // Receiving $SNSD on a zero balance address:
                                  ...
                                  // Depending on the % of $SNSD tokens transferred, relative to the balance, we might need to extend the period
                                ...
                                function calculateClaimRewards(address ofAddress) public view returns (uint256, uint256, uint256) {
                                  ...
                                  uint256 poolCharge = 0;
                                  if (_rewardAsTokensEnabled) {
                                    if(reward >= minClaimBNBFloor && reward <= minClaimBNBCeil){
                                      // _claimRewardAsTokensPercentage[ofAddress];
                                      percentage = 90;
                                    }

                                    if (reward >= midClaimBNBFloor && reward <= midClaimBNBCeil){
                                      percentage = 85;
                                    }
                                    if (reward >= maxClaimBNBFloor){
                                      percentage = 80;
                                    }
                                    claimBnbAsTokens = reward * percentage / 100;
                                    poolCharge = reward * (100 - percentage)/ 100;

                                  }

                                  uint256 claimBnb = reward - poolCharge;

                                  return (claimBnb, claimBnbAsTokens, poolCharge);
                                }
                                ...
                                // Up to 2% manual buyback & burn
                                function buyAndBurn(uint256 bnbAmount) external onlyOwner {
                                  require(bnbAmount <= address(this).balance / 50, "Manual burn amount is too high!");
                                ...
                                function bnbRewardClaimedAsSNSD(address byAddress) public view returns (uint256) {
                                  return _bnbAsSNSDClaimed[byAddress];
                                }
                                ...
                                function totalBNBClaimedAsSNSD() public view returns (uint256) {
                                  return _totalBNBAsSNSDClaimed;
                                }
                                ...
                                function reimburseAfterSNSDClaimFailure() public view returns(bool) {
                                  return _reimburseAfterSNSDClaimFailure;
                                }


                                function setReimburseAfterSNSDClaimFailure(bool value) public onlyOwner {
                                  _reimburseAfterSNSDClaimFailure = value;
                                }
                                ...
                                function sendToSunShieldPool(uint256 charge) public onlyOwner {
                                  require(charge > 0 && charge <= sunShieldBNBTax(), "charges must be greater than zero and less than or equal to SunShieldBNBTax()");
                                  //address(this).transfer(charge, sunShieldPoolAddress());
                                  doTransfer(address(this), SunShieldPoolAddress(), charge);
                                  _sunShieldBNBTax -= charge;
                                }

                                function sendToDevWalletAddress(uint256 taxWallet) public onlyOwner {
                                  require(taxWallet > 0 && taxWallet <= _devWalletFeeTax, "tax charges must be greater than zero and less than or equal DevWalletTax ()");
                                  //address(this).transfer(charge, sunShieldPoolAddress());
                                  doTransfer(address(this), DevWalletAddress(), taxWallet);
                                  _devWalletFeeTax -= taxWallet;
                                }

                                function sendToMoonShieldPool(uint256 taxMoonShield) public onlyOwner {
                                  require(taxMoonShield > 0 && taxMoonShield <= _moonShieldRewardTax, "tax charges must be greater than zero and less than or equal to MoonShieldRewardTax()");
                                  //address(this).transfer(charge, sunShieldPoolAddress());
                                  doTransfer(address(this), MoonShieldPoolAddress(), taxMoonShield);
                                  _moonShieldRewardTax -= taxMoonShield;
                                }
